...
model_mae = Gauge("ml_model_mae", "Current model MAE", ["model_name", "model_version"])

model_rmse = Gauge(
    "ml_model_rmse", "Current model RMSE", ["model_name", "model_version"]
)
...
active_models = Gauge("ml_active_models", "Number of active models by stage", ["stage"])
...
model_cache_hits = Counter(
    "ml_model_cache_hits_total", "Total number of model cache hits", ["model_name"]
)

model_cache_misses = Counter(
    "ml_model_cache_misses_total", "Total number of model cache misses", ["model_name"]
)
...
class MetricsCollector:
    ...
    def update_model_performance(
        model_name: str, model_version: str, r2_score: float, mae: float, rmse: float
    ):
        ...
        try:
            ...
            model_mae.labels(model_name=model_name, model_version=model_version).set(
                mae
            )

            model_rmse.labels(model_name=model_name, model_version=model_version).set(
                rmse
            )
    ...
    def record_training(
        ...
        try:
            training_duration.labels(model_type=model_type, status=status).observe(
                duration_seconds
            )

            training_counter.labels(
                model_type=model_type, status=status, trigger=trigger
            ).inc()
    ...
    @staticmethod
    def record_model_transition(model_name: str, from_stage: str, to_stage: str):
    ...
    @staticmethod
    def record_drift(model_name: str, feature_name: str, severity: str = "warning"):
        """Record a drift detection event."""
        try:
            drift_detected.labels(
                model_name=model_name, feature_name=feature_name, severity=severity
            ).inc()
    ...
    @staticmethod
    def record_performance_degradation(model_name: str, severity: str = "warning"):
