...
class Settings(BaseSettings):
    ...
    JWT_SECRET_KEY: str = os.getenv(
        "JWT_SECRET_KEY", "darkstore-secret-key-change-in-production-2026"
    )
    ...
    GOOGLE_GEOCODING_API_KEY: str = os.getenv(
        "GOOGLE_GEOCODING_API_KEY", os.getenv("GOOGLE_MAPS_API_KEY", "")
    )
    GOOGLE_DISTANCE_MATRIX_API_KEY: str = os.getenv(
        "GOOGLE_DISTANCE_MATRIX_API_KEY", os.getenv("GOOGLE_MAPS_API_KEY", "")
    )
    ...
    MLFLOW_ENABLE_TRACKING: bool = (
        os.getenv("MLFLOW_ENABLE_TRACKING", "True").lower() == "true"
    )

    # Model Serving Configuration
    MODEL_CACHE_TTL_SECONDS: int = int(os.getenv("MODEL_CACHE_TTL_SECONDS", "3600"))
    MODEL_RELOAD_CHECK_INTERVAL: int = int(
        os.getenv("MODEL_RELOAD_CHECK_INTERVAL", "60")
    )
    DEFAULT_MODEL_NAME: str = os.getenv(
        "DEFAULT_MODEL_NAME", "demand_forecasting_model"
    )
    ...
    def validate_secret_key(cls, v, values):
        ...
        if (
            ...
            and "change-in-production" in v.lower()
        ):
