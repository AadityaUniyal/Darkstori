...
# Use specific Distance Matrix API key, fallback to general Maps key
GOOGLE_DISTANCE_MATRIX_API_KEY = (
    settings.GOOGLE_DISTANCE_MATRIX_API_KEY or settings.GOOGLE_MAPS_API_KEY
)
...
class DistanceMatrixAPI:
    ...
    def __init__(self, api_key: str = GOOGLE_DISTANCE_MATRIX_API_KEY):
        self.api_key = api_key
        if not self.api_key:
            raise ValueError(
                "Google Distance Matrix API key not configured. Set GOOGLE_DISTANCE_MATRIX_API_KEY in .env"
            )
    ...
    def analyze_store_coverage(
        self, store_lat: float, store_lng: float, pincodes_df: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        result = pincodes_df.merge(
            coverage_df, left_on="pincode", right_on="destination_pincode", how="left"
        )
...
def main():
    ...
    store_location = {"name": "Blinkit Koramangala", "lat": 12.9352, "lng": 77.6245}
    ...
    coverage_df = api.analyze_store_coverage(
        store_location["lat"], store_location["lng"], sample_pincodes
    )
