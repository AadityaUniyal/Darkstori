...
class PerformanceMonitor:
    ...
    async def log_actual(
        ...
        try:
            ...
            if ml_prediction:
                ...
                ml_prediction.prediction_error = abs(actual_value - ml_prediction.prediction)
    ...
    async def calculate_rolling_metrics(
        ...
        try:
            ...
            if not predictions:
                logger.warning(f"No predictions found for {model_name} in last {window_days} days")
                return {}
        ...
        try:
            ...
            for column in training_data.select_dtypes(include=[np.number]).columns:
                ...
                drift_results.append({
                    ...
                    'training_mean': training_mean,
                    'current_mean': current_mean,
                    'mean_change_pct': ((current_mean - training_mean) / training_mean * 100) if training_mean != 0 else 0
                })
        ...
        try:
            ...
            if status in ['warning', 'alert'] and self.alert_manager:
                await self.alert_manager.send_performance_degradation_alert(
                    ...
                    model_version=model_version or 'all',
                    current_r2=r2_score,
                    threshold=alert_threshold if status == 'alert' else warning_threshold,
                    metrics=metrics
                )
    ...
    async def get_performance_history(
        ...
        try:
            ...
            query = select(MLPerformanceMetric).where(
                and_(
                    MLPerformanceMetric.model_name == model_name,
                    MLPerformanceMetric.window_days == window_days,
        ...
        try:
            ...
            for drift in drifts:
                history.append({
                    ...
                    'mean_change_pct': ((drift.current_mean - drift.training_mean) / drift.training_mean * 100) if drift.training_mean != 0 else 0
                })
