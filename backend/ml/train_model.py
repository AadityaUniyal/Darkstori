    ...
    try:
        ...
        async with get_async_session() as db_session:
            pipeline = MLflowTrainingPipeline(
                experiment_name=experiment_name, db_session=db_session
            )
            ...
            model_types = config.get("training", {}).get(
                "model_types", ["random_forest", "gradient_boosting", "xgboost"]
            )

            tags = {"environment": "production", "trigger": "manual", "version": "1.0"}

            results = await pipeline.run(model_types=model_types, tags=tags)
            ...
            print("\n" + "-" * 70)
            print(
                f"{'Model':<20} {'R² Score':<12} {'RMSE':<12} {'MAE':<12} {'MAPE':<12}"
            )
            print("-" * 70)
            ...
            for model_result in results.get("models", []):
                ...
                print(
                    f"{model_result.get('model_type', 'Unknown'):<20} "
            ...
            if best_model.get("registered", False):
                print(
                    f"[OK] Model registered: {best_model.get('model_name', 'Unknown')}"
                )
            ...
            mlflow_uri = config.get("mlflow", {}).get(
