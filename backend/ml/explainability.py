    ...
    def calculate_feature_importance(self) -> Dict[str, float]:
        ...
        try:
            ...
            if hasattr(self.model, 'feature_importances_'):
                ...
                logger.info(f"Calculated feature importance for {len(importance_dict)} features")
    ...
    def explain_prediction(
        ...
        try:
            ...
            if shap_values is not None:
                ...
                explanation["feature_contributions"] = dict(sorted_contributions[:top_n])
                explanation["top_positive_contributors"] = [
                    {"feature": k, "contribution": float(v), "value": float(X[k].iloc[0])}
                    for k, v in positive
                ]
