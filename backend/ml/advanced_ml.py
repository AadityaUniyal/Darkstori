...
class DemandPredictor:
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'order_count' in features.columns:
            ...
            features['order_count_rolling_7'] = features['order_count'].rolling(7).mean()
...
class ChurnPredictor:
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'avg_order_value' in features.columns:
            features['aov_trend'] = features.groupby('customer_id')['avg_order_value'].pct_change()
...
class GeospatialClustering:
    ...
    def identify_optimal_locations(
        ...
        cluster_stats = demand_points.groupby('cluster').agg({
            'latitude': 'mean',
            'longitude': 'mean',
            'demand': 'sum',
            'population': 'sum'
