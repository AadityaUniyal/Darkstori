...
class PredictionService:
    ...
    async def predict_batch(
        ...
        try:
            ...
            for i in range(0, len(input_df), chunk_size):
                ...
                logger.info(f"Processed {min(i+chunk_size, len(input_df))}/{len(input_df)} rows")
            ...
            return {
                ...
                'model_name': model_name,
                'model_version': str(self.model_loader.get_model_info(model_name).get('version', 'unknown'))
            }
    ...
    def _validate_batch_input(self, df: pd.DataFrame) -> None:
        ...
        """
        required_cols = [
            'pincode', 'order_date', 'population', 'coverage_score',
            'city_tier', 'city', 'state'
        ]
