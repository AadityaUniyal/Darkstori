...
class BaselineModels:
    ...
    def train_all_baselines(
        ...
        try:
            ...
            # Median predictor
            median_results = self._train_median_predictor(X_train, y_train, X_test, y_test)
            self.baseline_results['median_predictor'] = median_results
            
            # Simple linear regression
            linear_results = self._train_linear_regression(X_train, y_train, X_test, y_test)
            ...
            if self._is_time_series(X_train):
                last_value_results = self._train_last_value_predictor(X_train, y_train, X_test, y_test)
    ...
    def _train_mean_predictor(
        ...
        try:
            ...
            logger.info(f"Mean predictor - R²: {metrics['r2_score']:.4f}, RMSE: {metrics['rmse']:.2f}")
    ...
    def _train_median_predictor(
        ...
        try:
            ...
            logger.info(f"Median predictor - R²: {metrics['r2_score']:.4f}, RMSE: {metrics['rmse']:.2f}")
    ...
    def _train_linear_regression(
        ...
        try:
            ...
            logger.info(f"Linear regression - R²: {metrics['r2_score']:.4f}, RMSE: {metrics['rmse']:.2f}")
    ...
    def _train_last_value_predictor(
        ...
        try:
            ...
            logger.info(f"Last value predictor - R²: {metrics['r2_score']:.4f}, RMSE: {metrics['rmse']:.2f}")
    ...
    def generate_comparison_chart(
        ...
        try:
            ...
            r2_scores = [
                ...
                for metrics in self.baseline_results.values()
...
def log_baseline_comparisons_to_mlflow(
    ...
    try:
        ...
        # Log improvements
        for baseline_name, improvement_pct in comparison.get('improvements', {}).items():
            mlflow_client.log_metric(
                run_id,
                f"improvement_over_{baseline_name}_pct",
                improvement_pct
            )
        ...
        baseline_models.generate_improvement_chart(model_name, model_metrics, improvement_path)
        mlflow_client.log_artifact(run_id, improvement_path)
        
        # Log tags
        mlflow_client.set_tag(run_id, "beats_all_baselines", str(comparison.get('beats_all_baselines', False)))
        mlflow_client.set_tag(run_id, "best_baseline", comparison.get('best_baseline', 'unknown'))
