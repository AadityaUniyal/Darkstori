...
class ModelRegistry:
    ...
    def get_latest_model(self, name: str, stage: Optional[str] = None) -> Optional[ModelVersion]:
    ...
    def search_models(self, filter_string: Optional[str] = None) -> List[RegisteredModel]:
    ...
    def get_model_info(self, name: str, version: Optional[int] = None, stage: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "creation_timestamp": datetime.fromtimestamp(model_version.creation_timestamp / 1000).isoformat(),
                "last_updated_timestamp": datetime.fromtimestamp(model_version.last_updated_timestamp / 1000).isoformat(),
    ...
    def promote_model(
        ...
        try:
            ...
            if model_version.current_stage != from_stage:
                raise ValueError(f"Model version {version} is in {model_version.current_stage}, not {from_stage}")
            
            # Transition to new stage
            return self.transition_model_stage(name, version, to_stage, archive_existing=True)
            
        except Exception as e:
            logger.error(f"Failed to promote model: {e}")
            raise
    
    def load_model(self, name: str, version: Optional[int] = None, stage: Optional[str] = "Production") -> Any:
