...
class EvaluationEngine:
    ...
    def evaluate_regression(
        ...
        try:
            metrics = {
                ...
                f"{dataset_name}_mape": mean_absolute_percentage_error(y_true, y_pred) * 100
            }
    ...
    def cross_validate(
        ...
        try:
            ...
            logger.info(f"Cross-validation ({cv} folds): {scoring} = {scores.mean():.4f} ± {scores.std():.4f}")
            ...
            for tier in tiers.unique():
                ...
                if len(tier_y_true) > 0:
                    tier_metrics[tier] = {
                        ...
                        "mape": mean_absolute_percentage_error(tier_y_true, tier_y_pred) * 100,
    ...
    def _plot_feature_importance(self, feature_importance: pd.DataFrame, top_n: int = 20) -> None:
    ...
    def validate_model(
        ...
        try:
            ...
            if self.tracker:
                ...
                self.tracker.set_tag("validation_status", "passed" if is_valid else "failed")
