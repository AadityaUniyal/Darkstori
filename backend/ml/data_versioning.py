...
class DataVersionManager:
    ...
    def detect_schema_changes(
        self, current_schema: Dict[str, str], previous_schema: Dict[str, str]
    ) -> Dict[str, Any]:
        """
    ...
    def _build_lineage_graph(
        self, sources: List[Dict[str, Any]], transformations: List[str]
    ) -> Dict[str, Any]:
...
def log_dataset_version_to_mlflow(
    ...
    try:
        ...
        mlflow_client.log_param(
            run_id, f"{dataset_name}_version", version_info["version_id"]
        )
        mlflow_client.log_param(
            run_id, f"{dataset_name}_hash", version_info["dataset_hash"]
        )
        mlflow_client.log_param(
            run_id, f"{dataset_name}_rows", version_info["num_rows"]
        )
        mlflow_client.log_param(
            run_id, f"{dataset_name}_columns", version_info["num_columns"]
        )
        ...
        # Log metadata
        metadata_path = str(
            Path(version_info["artifact_path"]).parent
            / f"{version_info['version_id']}_metadata.json"
        )
