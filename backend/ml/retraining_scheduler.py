...
from database.models.models import MLPerformanceMetric, MLTrainingJob
...
class RetrainingScheduler:
    ...
    async def should_retrain(
        self, model_name: str, reason: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        try:
            decision = {"should_retrain": False, "reasons": [], "metrics": {}}
            ...
            return {"should_retrain": False, "reasons": [], "error": str(e)}
    ...
    async def trigger_retraining(
        ...
        try:
            ...
            job_id = await self._create_training_job(
                model_name=model_name, trigger="automated", config=config
            )

            # Initialize training pipeline
            pipeline = MLflowTrainingPipeline(
                experiment_name=experiment_name, db_session=self.db_session
            )
    ...
    async def _check_data_growth(self, model_name: str) -> float:
        ...
        try:
            ...
            query = (
                select(func.count())
                .select_from(MLPerformanceMetric)
                .where(
            ...
            growth_pct = (
                new_predictions / last_dataset_size if last_dataset_size > 0 else 0
            )
    ...
    async def _check_performance_degradation(self, model_name: str) -> Optional[str]:
        ...
        try:
            ...
            degradation = await self.monitor.check_performance_degradation(
                model_name=model_name, window_days=7
            )
    ...
    async def _create_training_job(
        self, model_name: str, trigger: str, config: Optional[Dict[str, Any]]
    ) -> str:
        """Create training job record."""
        try:
            job = MLTrainingJob(
    ...
    async def _evaluate_promotion(
        self, model_name: str, new_model_metrics: Dict[str, float]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"should_promote": False, "reason": f"Error: {str(e)}"}

    async def _promote_model(
        self, model_name: str, new_version: Optional[str], reason: str
    ) -> None:
        """
    ...
    async def _send_retraining_notification(
        self, model_name: str, results: Dict[str, Any], promoted: bool
    ) -> None:
...
async def run_scheduled_retraining(
    model_names: list[str], db_session: AsyncSession, check_interval_hours: int = 24
) -> None:
    """
