...
class ModelLoader:
            ...
            with self._lock:
                ...
                if not force_reload and model_name in self._cache:
                    ...
                    if age < self.cache_ttl_seconds:
                        logger.debug(f"Using cached model: {model_name} v{version} (age: {age:.0f}s)")
    ...
    def _load_artifacts(self, model_name: str, version: str) -> Tuple[Any, list]:
        ...
        try:
            ...
            try:
                import json
                features_path = client.download_artifacts(run_id, "engineered_features.json")
        ...
        try:
            with self._lock:
                ...
                if latest_version != cached_version:
                    logger.info(f"Model version changed: {cached_version} -> {latest_version}")
    ...
    def get_model_info(self, model_name: str) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                model, scaler, feature_names, version, timestamp = self._cache[model_name]
    ...
    async def auto_reload_loop(self, model_name: str, check_interval: int = 60) -> None:
        ...
        """
        try:
            logger.info(f"Starting auto-reload loop for {model_name} (interval: {check_interval}s)")
