...
from backend.database.models import PincodeCoverage, OrderSynthetic
...
async def get_coverage_gaps(
    ...
    query = select(PincodeCoverage).where(
        PincodeCoverage.population >= min_population,
        PincodeCoverage.coverage_score <= max_coverage
    ).order_by(PincodeCoverage.population.desc()).limit(limit)
