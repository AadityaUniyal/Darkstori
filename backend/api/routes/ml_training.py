...
from backend.database.models import MLTrainingJob
...
async def list_training_jobs(
    ...
    """
    try:
        query = select(MLTrainingJob).order_by(
            MLTrainingJob.started_at.desc()
        ).limit(limit)
    ...
    try:
        ...
        for exp in experiments:
            ...
            experiment_list.append({
                ...
                'creation_time': datetime.fromtimestamp(exp.creation_time / 1000).isoformat() if exp.creation_time else None
            })
...
async def get_experiment_runs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve experiment runs"
        )
