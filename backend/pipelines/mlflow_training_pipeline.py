...
class MLflowTrainingPipeline:
    ...
    def __init__(self, experiment_name: str = "demand_forecasting"):
        ...
        logger.info(f"MLflowTrainingPipeline initialized for experiment: {experiment_name}")
    ...
    def run(
        ...
        try:
            ...
            X_train, X_test, y_train, y_test, feature_names = self.feature_pipeline.prepare_features(
                df=df,
            ...
            logger.info(f"Training completed. Best model: {best_model_type} (R²={best_result['test_r2']:.4f})")
    ...
    def train_model(
        ...
        try:
            ...
            train_metrics = self.evaluator.evaluate_regression(y_train, y_train_pred, "train")
            
            # Evaluate on test set
            test_metrics = self.evaluator.evaluate_regression(y_test, y_test_pred, "test")
