...
from src.utils.helpers import validate_coordinates, clean_platform_name
...
class DataPipeline:
    ...
    def clean_data(self, data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        for key in cleaned:
            if 'pincode' in cleaned[key].columns:
                cleaned[key]['pincode'] = cleaned[key]['pincode'].astype(str).str.zfill(6)
    ...
    def merge_datasets(self, data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        if 'population' in data:
            df = df.merge(
                data['population'][['pincode', 'population']],
                on='pincode',
                how='left'
            )
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'order_date' in features.columns:
            ...
            if features['order_date'].dt.hour.notna().any():
                ...
                features['is_peak_hour'] = features['hour'].isin([12, 13, 19, 20, 21]).astype(int)
        ...
        if 'order_count' in features.columns:
            ...
            features['order_count_lag1'] = features.groupby('pincode')['order_count'].shift(1)
            features['order_count_lag7'] = features.groupby('pincode')['order_count'].shift(7)
            features['order_count_lag30'] = features.groupby('pincode')['order_count'].shift(30)
            
            # Rolling statistics
            features['order_count_rolling_7'] = features.groupby('pincode')['order_count'].rolling(7).mean().reset_index(0, drop=True)
            features['order_count_rolling_30'] = features.groupby('pincode')['order_count'].rolling(30).mean().reset_index(0, drop=True)
        ...
        if 'population' in features.columns and 'coverage_score' in features.columns:
            features['pop_coverage_interaction'] = features['population'] * features['coverage_score']
