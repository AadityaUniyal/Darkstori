...
from backend.pipelines.training_pipeline import ModelTrainer
...
class PredictionPipeline:
    ...
    def prepare_input_data(self, input_data: pd.DataFrame) -> pd.DataFrame:
        ...
        available_features = [
            f for f in self.model_trainer.feature_names if f in processed_data.columns
        ]
    ...
    def predict(self, input_data: pd.DataFrame, model_name: str = None) -> np.ndarray:
        """
        Make predictions.
    ...
    def predict_with_confidence(
        self, input_data: pd.DataFrame, confidence_level: float = 0.95
    ) -> pd.DataFrame:
        """
    ...
    def forecast_future(
        self, base_data: pd.DataFrame, periods: int = 30, freq: str = "D"
    ) -> pd.DataFrame:
        """
        ...
        future_data = pd.DataFrame({"order_date": future_dates})
        ...
        for col in base_data.columns:
            if col != "order_date" and col not in future_data.columns:
                ...
                if base_data[col].dtype in [np.float64, np.int64]:
                    ...
                    future_data[col] = (
                        base_data[col].mode()[0]
                        if len(base_data[col].mode()) > 0
                        else None
                    )
    ...
    def batch_predict(self, input_file: str, output_file: str, batch_size: int = 1000):
        """
    ...
    def explain_prediction(self, input_data: pd.DataFrame, index: int = 0) -> Dict:
        """
        ...
        if hasattr(self.model_trainer.best_model, "feature_importances_"):
            ...
            return {"prediction": prediction, "top_features": []}
