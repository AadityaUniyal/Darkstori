...
from src.utils.config import RAW_DATA_DIR, EXTERNAL_DATA_DIR
...
class DataCollector:
    ...
    def generate_synthetic_pincode_data(self) -> pd.DataFrame:
        ...
        cities_data = [
            # Metro cities
            {'city': 'Mumbai', 'state': 'Maharashtra', 'pincode_start': 400001, 'count': 100, 'tier': 'Metro', 'pop_range': (150000, 250000)},
            {'city': 'Delhi', 'state': 'Delhi', 'pincode_start': 110001, 'count': 100, 'tier': 'Metro', 'pop_range': (150000, 250000)},
            {'city': 'Bangalore', 'state': 'Karnataka', 'pincode_start': 560001, 'count': 100, 'tier': 'Metro', 'pop_range': (150000, 250000)},
            {'city': 'Hyderabad', 'state': 'Telangana', 'pincode_start': 500001, 'count': 80, 'tier': 'Metro', 'pop_range': (140000, 230000)},
            {'city': 'Chennai', 'state': 'Tamil Nadu', 'pincode_start': 600001, 'count': 80, 'tier': 'Metro', 'pop_range': (140000, 230000)},
            {'city': 'Kolkata', 'state': 'West Bengal', 'pincode_start': 700001, 'count': 80, 'tier': 'Metro', 'pop_range': (140000, 230000)},
            {'city': 'Pune', 'state': 'Maharashtra', 'pincode_start': 411001, 'count': 60, 'tier': 'Metro', 'pop_range': (130000, 220000)},
            {'city': 'Ahmedabad', 'state': 'Gujarat', 'pincode_start': 380001, 'count': 60, 'tier': 'Metro', 'pop_range': (130000, 220000)},
            
            # Tier 1 cities
            {'city': 'Jaipur', 'state': 'Rajasthan', 'pincode_start': 302001, 'count': 40, 'tier': 'Tier1', 'pop_range': (100000, 180000)},
            {'city': 'Lucknow', 'state': 'Uttar Pradesh', 'pincode_start': 226001, 'count': 40, 'tier': 'Tier1', 'pop_range': (100000, 180000)},
            {'city': 'Chandigarh', 'state': 'Chandigarh', 'pincode_start': 160001, 'count': 30, 'tier': 'Tier1', 'pop_range': (90000, 160000)},
            {'city': 'Indore', 'state': 'Madhya Pradesh', 'pincode_start': 452001, 'count': 35, 'tier': 'Tier1', 'pop_range': (95000, 170000)},
            {'city': 'Kochi', 'state': 'Kerala', 'pincode_start': 682001, 'count': 30, 'tier': 'Tier1', 'pop_range': (90000, 160000)},
            {'city': 'Coimbatore', 'state': 'Tamil Nadu', 'pincode_start': 641001, 'count': 30, 'tier': 'Tier1', 'pop_range': (90000, 160000)},
            
            # Tier 2 cities
            {'city': 'Bhopal', 'state': 'Madhya Pradesh', 'pincode_start': 462001, 'count': 25, 'tier': 'Tier2', 'pop_range': (60000, 120000)},
            {'city': 'Nagpur', 'state': 'Maharashtra', 'pincode_start': 440001, 'count': 25, 'tier': 'Tier2', 'pop_range': (60000, 120000)},
            {'city': 'Vadodara', 'state': 'Gujarat', 'pincode_start': 390001, 'count': 20, 'tier': 'Tier2', 'pop_range': (55000, 110000)},
            {'city': 'Mysore', 'state': 'Karnataka', 'pincode_start': 570001, 'count': 20, 'tier': 'Tier2', 'pop_range': (55000, 110000)},
            
            # Tier 3 cities
            {'city': 'Dehradun', 'state': 'Uttarakhand', 'pincode_start': 248001, 'count': 15, 'tier': 'Tier3', 'pop_range': (30000, 80000)},
            {'city': 'Shimla', 'state': 'Himachal Pradesh', 'pincode_start': 171001, 'count': 10, 'tier': 'Tier3', 'pop_range': (25000, 70000)},
        ]
    ...
    def generate_synthetic_orders_data(self, pincodes_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, pincode_row in sample_pincodes.iterrows():
            for date in dates:
                ...
                base_orders = (pincode_row['population'] / 1000) * tier_multiplier.get(pincode_row['city_tier'], 0.5)
                ...
                growth_multiplier = 1 + (days_from_start / 365) * 0.3  # 30% annual growth
                
                # Add randomness
                order_count = int(base_orders * day_multiplier * growth_multiplier * np.random.uniform(0.7, 1.3))
            ...
            for _, pincode_row in store_pincodes.iterrows():
                ...
                for platform in selected_platforms:
                    all_stores.append({
                        ...
                        'pincode': pincode_row['pincode'],
                        'city_tier': pincode_row['city_tier'],
                        'latitude': pincode_row['latitude'] + np.random.uniform(-0.01, 0.01),
                        'longitude': pincode_row['longitude'] + np.random.uniform(-0.01, 0.01),
                        'is_active': True,
                        'opening_date': datetime.now() - timedelta(days=np.random.randint(30, 730))
                    })
                    store_id += 1
    ...
    def generate_coverage_data(self, pincodes_df: pd.DataFrame, stores_df: pd.DataFrame) -> pd.DataFrame:
