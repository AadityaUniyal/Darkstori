...
def check_mlflow_tables(engine) -> bool:
    ...
    logger.info(f"Found {len(mlflow_tables_found)}/{len(required_tables)} MLflow tables")
    ...
    try:
        ...
        for exp_config in experiments:
            ...
            try:
                ...
                if experiment:
                    logger.info(f"  Experiment '{exp_name}' already exists (ID: {experiment.experiment_id})")
                else:
                    # Create new experiment
                    exp_id = client.create_experiment(
                        name=exp_name,
                        tags={"description": exp_description}
                    )
