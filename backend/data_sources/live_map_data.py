...
from src.utils.helpers import logger, retry_on_failure
...
class LiveMapDataFetcher:
    ...
    def fetch_live_stores(self, lat: float, lng: float, radius: int = 5000) -> List[Dict]:
        """
    ...
    @retry_on_failure(max_retries=3)
    def fetch_live_traffic(self, origin: Tuple[float, float], 
        ...
        try:
            ...
            if data['status'] == 'OK':
                ...
                return {
                    ...
                    'duration_in_traffic_mins': route.get('duration_in_traffic', {}).get('value', 0) / 60,
                    'traffic_delay_mins': (route.get('duration_in_traffic', {}).get('value', 0) - 
