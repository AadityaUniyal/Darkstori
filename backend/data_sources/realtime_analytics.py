...
class RealtimeAnalytics:
    ...
    def _calculate_rate(self) -> float:
        ...
        time_diff = (last_order['timestamp'] - first_order['timestamp']).total_seconds() / 60
    ...
    def get_live_metrics(self) -> Dict:
        ...
        if len(self.metrics_history) >= 2:
            ...
            order_trend = ((latest['total_orders'] - previous['total_orders']) / 
                          previous['total_orders'] * 100) if previous['total_orders'] > 0 else 0
            
            value_trend = ((latest['avg_order_value'] - previous['avg_order_value']) / 
                          previous['avg_order_value'] * 100) if previous['avg_order_value'] > 0 else 0
        else:
            order_trend = 0
            value_trend = 0
    ...
    def generate_live_forecast(self, horizon_minutes: int = 30) -> pd.DataFrame:
        ...
        forecast_times = [last_time + timedelta(minutes=i) for i in range(1, horizon_minutes + 1)]
    ...
    def get_platform_performance(self) -> pd.DataFrame:
        ...
        performance = df.groupby('platform').agg({
            'order_value': ['count', 'mean', 'sum'],
            'delivery_time': 'mean' if 'delivery_time' in df else lambda x: 0
        }).reset_index()
        
        performance.columns = ['platform', 'order_count', 'avg_order_value', 
                              'total_revenue', 'avg_delivery_time']
