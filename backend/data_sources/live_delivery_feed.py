...
class LiveDeliveryFeed:
    ...
    def _get_demand_factor(self, platform: str, pincode: str) -> float:
        ...
        recent_orders = [
            d for d in self.delivery_stream
            if d['platform'] == platform and d['pincode'] == pincode
    ...
    def get_live_metrics(self) -> Dict:
        ...
        # Calculate metrics
        metrics = {
            'total_deliveries_last_hour': len(df[
                (datetime.now() - df['timestamp']).dt.seconds < 3600
            ]),
            'avg_delivery_time': df['delivery_time'].mean() if 'delivery_time' in df else 0,
            'platform_breakdown': df['platform'].value_counts().to_dict() if 'platform' in df else {},
            'busiest_pincodes': df['pincode'].value_counts().head(10).to_dict() if 'pincode' in df else {},
        ...
        report = {
            ...
            'platforms': {
                platform: {
                    ...
                    'avg_time': daily_df[daily_df['platform'] == platform]['delivery_time'].mean()
                    if 'delivery_time' in daily_df else 0,
                    'market_share': len(daily_df[daily_df['platform'] == platform]) / len(daily_df) * 100
                }
                for platform in daily_df['platform'].unique()
            },
            'peak_hours': daily_df.groupby(daily_df['timestamp'].dt.hour).size().to_dict(),
            'top_pincodes': daily_df['pincode'].value_counts().head(20).to_dict() if 'pincode' in daily_df else {},
    ...
    def _generate_insights(self, df: pd.DataFrame) -> List[str]:
        ...
        if 'delivery_time' in df:
            ...
            if avg_time > 20:
                insights.append(f"⚠️ Average delivery time is high ({avg_time:.1f} mins)")
            elif avg_time < 12:
